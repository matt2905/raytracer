...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/03/17 18:37:02 by mmartin           #+#    #+#             */
/*   Updated: 2014/03/17 19:23:00 by mmartin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_obj	ft_find_coef(t_object obj, t_vector dir, t_vec v)
{
	...
	h.b = 2 * (pow(cos(obj.misc), 2) * ft_vector_dot(v.e, v.b));
	h.b -= 2 * (pow(sin(obj.misc), 2) * alpha * beta);
...
double			ft_hyperboloid(t_object obj, t_vector dir, t_vector o)
{
	...
	if ((h.delta = pow(h.b, 2) - 4 * h.a * h.c) >= 0)
	{
		h.alpha1 = (-h.b - sqrt(h.delta)) / (2.0 * h.a);
		h.alpha2 = (-h.b + sqrt(h.delta)) / (2.0 * h.a);
